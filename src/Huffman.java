...
below:
Class Huffman:
• frequencyCount(String text): Compute the frequency of each character in the input string.
• buildHuffman(): Use a min-heap to construct the Huffman tree where each leaf node
represents a character and the path from root to leaf defines its binary code.
• genCode(): From the constructed tree, generate a mapping of each character to its binary code.
(Note: The total number of elements mapped should be text.length)
• encode(String text): Replace each character in the input string with its corresponding Huffman
code and return the encoded text.
• printStats(): Print to console: the frequency table (character and frequency), the Huffman code
mapping for each character, and the encoded binary string..
